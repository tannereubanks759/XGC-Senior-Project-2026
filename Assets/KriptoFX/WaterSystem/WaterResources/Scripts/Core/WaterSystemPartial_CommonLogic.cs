...
namespace KWS
{
    public partial class WaterSystem
    {
        ...
        private List<Vector3>            _underwaterPositions = new List<Vector3>();
        ...
        internal static void RequestUnderwaterState(HashSet<Camera> cameras)
        {
            ...
            foreach (var stateCamera in _underwaterStateCameras)
            {
                ...
                var windSpeed = 0.1f + Mathf.Clamp01(WaterSharedResources.GlobalSettings.CurrentWindSpeed / 15.0f);

                stateCamera.Value.CameraNearPlanePoints[4] = stateCamera.Value.CameraNearPlanePoints[0] + Vector3.down * windSpeed; //underwater prediction because async readback can't have 100% accuracy position. 
            ...
            foreach (var stateCamera in _underwaterStateCameras)
            {
                ...
                stateCamera.Value.IsCameraRequireWaterDrops = (points[0].y < result[0].Position.y + 1f) && result[0].Foam > 0.5f;
        ...
        internal bool IsIntersectRiver(Vector3 worldPosition)
        {
            return SplineMeshComponent.IsIntersectRiver(this, worldPosition, 1);
        }
