...
namespace KWS
{
    internal class ScreenSpaceReflectionPass : WaterPass
    {
        ...
        public void ExecuteRaymarchedReflection(WaterPass.WaterPassContext waterContext)
        {
            ...
            if (ssrDatas.Count > MaxSsrDataCameras)
            {
                ReleaseSsrDatas();
                ssrDatas.Clear();
            }
            ...
            cmd.SetGlobalInt(KWS_ShaderConstants.ConstantWaterParams.UseScreenSpaceReflectionSky, settings.UseScreenSpaceReflectionSky ? 1 : 0);
