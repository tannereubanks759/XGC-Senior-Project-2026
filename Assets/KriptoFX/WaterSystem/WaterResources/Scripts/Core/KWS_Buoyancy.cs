...
namespace KWS
{
    ...
    public class KWS_Buoyancy : MonoBehaviour
    {
        ...
        [Range(0, 20)]
        public float VelocityForce = 2.0f;
        ...
        private List<Vector3> _waterWorldPos;
        ...
        private void OnEnable()
        {
            ...
            _waterWorldPos = KW_Extensions.InitializeListWithDefaultValues(voxels.Count, Vector3.zero);
        ...
        private void FixedUpdate()
        {
            ...
            if (_request.Result.Count == 0)
            {
                _rigidBody.Sleep();
                return;
            }

            var riverWaterLevelOffset = WaterSystem.GetRiverOffset(transform.position);
            ...
            for (int i = 0; i < _voxelsWorldPos.Length; i++)
            {
                ...
                var waterPos      = _request.Result[i].Position + Vector3.up * riverWaterLevelOffset;
                var waterVelocity = new Vector3(Mathf.Clamp(_request.Result[i].Velocity.x, -1, 1), 0, Mathf.Clamp(_request.Result[i].Velocity.y, -1, 1));
               

                var velocity = _rigidBody.GetPointVelocity(wp);
                velocity += -waterVelocity * VelocityForce;
...
#if UNITY_EDITOR
                if (_isSelectedFrames > 0) debugForces.Add(new[] { wp, new Vector3(_request.Result[i].Velocity.x, 0, _request.Result[i].Velocity.y) }); // For drawing force gizmos
#endif
                _isSelectedFrames--;
            }
