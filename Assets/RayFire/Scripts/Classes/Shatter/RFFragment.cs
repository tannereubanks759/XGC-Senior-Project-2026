...
{
    /// <summary>
    /// V1 engine class for Rayfire Shatter component.
    /// </summary>
    public static class RFFragment
    {
        ...
        static bool SetRigidShatter(RayfireRigid scr)
        {
            ...
            {
                // Disable clusters with runtime caching. Crash!
                if (scr.mshDemol.ch.MultiFrameState == true )
                    if (scr.mshDemol.sht != null)
        ...
        static void InteractiveCreate(RayfireShatter scr)
        {
            if (scr.intGo == null)
            {
                scr.intGo = new GameObject (scr.name + "_Interactive");
                scr.intMf = scr.intGo.AddComponent<MeshFilter>();
            }
            
            if (scr.intMf == null)
            {
                scr.intMf = scr.intGo.GetComponent<MeshFilter>();
                if (scr.intMf == null)
                    scr.intMf = scr.intGo.AddComponent<MeshFilter>();
            }
            
            // Copy tm
            scr.intGo.transform.position = scr.transForm.position;
                
            scr.intGo.tag   = scr.gameObject.tag;
            scr.intGo.layer = scr.gameObject.layer;
            
            if (scr.meshRenderer != null)
            {
                if (scr.intMr == null)
                {
                    scr.intMr                 = scr.intGo.AddComponent<MeshRenderer>();
                    scr.intMr.sharedMaterials = scr.meshRenderer.sharedMaterials;
                }
            }
            else if (scr.skinnedMeshRend != null)
            {
...
namespace RayFire
{
    public static class RFFragment
    {
        ...
        public static void CacheMeshesMult(Transform tmSaved, ref List<Mesh> meshesList, ref List<Vector3> pivotsList, ref List<RFDictionary> subList, RayfireRigid scrRigid, List<int> batchAmount, int batchInd) {BuildTest();}
        public static void CacheMeshesInst(ref Mesh[] meshes, ref Vector3[] pivots, ref RFDictionary[] origSubMeshIdsRf, RayfireRigid scrRigid)  {BuildTest();}
        ...
        public static void SliceMeshes(ref Mesh[] meshes, ref Vector3[] pivots, ref RFDictionary[] origSubMeshIdsRf, RayfireRigid scrRigid, List<Vector3> sliceData)  {BuildTest();}
        public static void RescaleMesh (Mesh mesh, float scale){BuildTest();}
        public static bool InputMesh(RayfireRigid scr)
        {
            BuildTest();
            scr.meshes = null;
        ...
        public static void InteractiveStart(RayfireShatter scr) {BuildTest();}
        public static void InteractiveChange(RayfireShatter scr) {BuildTest();}
